...
%v5: changing the spectrogram parameters to 200,150 and changing the
%normalisation window to 0 to 2000

userfol="/Volumes/Users/mervyn";
savefolder="/seseyev5_1/";
...
folder=strcat("/Volumes/Hippocampus/Data/picasso-misc/",date,"/",session,"/",array,"/",channel);

if exist(strcat("/Volumes/Hippocampus/Data/picasso-misc/",date,"/session01/",array,"/",channel), "dir")==7 % Check that the folder exists
...
%     
    ...
    for n = 1:length(rp.data.markers) % trial marker
        ...
        datam = mean(data);
        [~,spec(T).F,spec(T).T,spec(T).P]=spectrogram(data-datam,200,150,(0:50),1000,'yaxis');
    ...
    for s=1:39 % limit the data from 0s to 2s only (average mean length of the trials)
    ...
    %power spectrum
    localpowerspectrum(mvlspec,1,50,c1,c2);
...
Pnorm1=mean(mspec.Pnorm(:,19:39),2); % Cue-onset
Pnorm2=mean(mspec.Pnorm(:,39:59),2); % Pure movement
Pnorm3=mean(mspec.Pnorm(:,29:49),2); % Movement-onset
Pnorm4=mean(mspec.Pnorm(:,27:31),2); % Mid-cue
